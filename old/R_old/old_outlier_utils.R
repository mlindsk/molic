...
old_neq_empt_chr <- function(x) !identical(x, character(0))
old_neq_empt_num <- function(x) !identical(x, numeric(0))


old_na <- function(df, a) {
  # The a- marginal table
  if( !old_neq_empt_chr(a) ) return(nrow(df)) # vars = NULL
...
old_Hx_ <- function(x) old_Gx_(x-1) - old_Gx_(x) # H

old_TY <- function(y, C_marginals, S_marginals) {
  # y: _named_ vector/observation
  Cs <- sapply(C_marginals, function(x) {
    old_na_ya(x, paste0(y[attr(x, "vars")], collapse = ""))
  })
  Ss <- if( length(S_marginals) >1) sapply(S_marginals[-1], function(x) {
    old_na_ya(x, paste0(y[attr(x, "vars")], collapse = ""))
  }) else 0L
  sum(old_Hx_(Cs) - c(0L, old_Hx_(Ss))) # H(C_yk) - H(S_yk)
}

old_sim_TY <- function(df,
                  C_marginals,
                  S_marginals,
                  nsim            = 1000,
                  ncores          = 1,
  ...
  if(!( length(C_marginals) - 1L)) {
    ...
    return( sapply(yC1_sim, old_TY, C_marginals, S_marginals) )
  } 
  ...
  y <- foreach::`%dopar%`(foreach::foreach(z = 1:nsim, .combine = 'c'), {
    ...
    for( k in 2:length(C_marginals) ) {
      ...
      if( is.null(Sk_vars) ) {
        ...
        ySk                 <- y_sim_z[Sk_idx]
        nSk_ySk             <- old_na_ya(nSk, ySk)
        nCk_given_Sk        <- old_na_b(nCk, structure(Sk_idx_in_Ck, names = ySk) )
    ...
    old_TY(structure(y_sim_z, names = Delta), C_marginals, S_marginals)    
  })
  doParallel::stopImplicitCluster()
  y
}

## old_sim_Y <- function(df,
##                   C_marginals,
##                   S_marginals,
##                   nsim            = 1000,
##                   ncores          = 1,
